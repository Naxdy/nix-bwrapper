    ...
    script.preCmds.stage2 = ''
      set_up_dbus_proxy() {
        ${pkgs.bubblewrap}/bin/bwrap \
          ...
          --clearenv \
          -- \
          ${pkgs.xdg-dbus-proxy}/bin/xdg-dbus-proxy "$DBUS_SESSION_BUS_ADDRESS" "$XDG_RUNTIME_DIR/app/${config.app.id}/bus" ${lib.optionalString cfg.logging "--log"} ${cfg.session.args}
      }
      ...
      set_up_system_dbus_proxy() {
        ${pkgs.bubblewrap}/bin/bwrap \
          ...
          --clearenv \
          -- \
          ${pkgs.xdg-dbus-proxy}/bin/xdg-dbus-proxy unix:path=/run/dbus/system_bus_socket "$XDG_RUNTIME_DIR/app/${config.app.id}/bus_system" ${lib.optionalString cfg.logging "--log"} ${cfg.system.args}
      }
